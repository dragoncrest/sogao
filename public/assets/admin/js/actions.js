...
$(document).ready(function(){
    ...
    /* eof alert delete category */

    /* alert delete QA */
    $.deleteQA = function(id){
        if (confirm("Xóa câu hỏi mã "+id) == true) {
            link = deleteUrl + '/' + id;
            $.ajax({
                type: "GET",
                url: link,
                success: function (data) {
                    if (data.status) {
                        $('tr#'+id).fadeOut(500, 'swing', function(){
                            $('tr#'+id).remove();
                        });
                    }
                },
...
function add_loader(block){
    var bW = $(block).width();
    var bH = $(block).height();
    var li = window.location.hostname + 'assets/images/';
    $(block).append('<div class="loader" style="width: '+bW+'px; height: '+bH+'px;"><img src="'+li+'/loader.gif"/></div>');
}
